/*
 * @Author: h-huan
 * @Date: 2023-04-28 09:26:38
 * @LastEditors: h-huan
 * @LastEditTime: 2023-04-28 09:26:38
 * @Description: 
 */
/*
 * @Author: h-huan
 * @Date: 2023-04-23 09:04:58
 * @LastEditors: h-huan
 * @LastEditTime: 2023-04-25 15:23:44
 * @Description: 
 */
...
function CRUD(options: any) {
  ...
  const methods = {
    ...
    */
    ...
    //   if (!(callVmHook(crud, CRUD.HOOK.beforeToEdit) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud.form))) {
    //     return
    //   }
    //   crud.status.edit = CRUD.STATUS.PREPARED
    //   crud.getDataStatus(crud.getDataId(data)).edit = CRUD.STATUS.PREPARED
    //   callVmHook(crud, CRUD.HOOK.afterToEdit)
    //   callVmHook(crud, CRUD.HOOK.afterToCU)
    // },
    // /**
    //  * 启动删除
    //  * @param {*} data 数据项
    ...
    //  */
    // cancelDelete(data) {
    //   if (!callVmHook(crud, CRUD.HOOK.beforeDeleteCancel)) {
    //     return
    //   }
    //   crud.getDataStatus(crud.getDataId(data)).delete = CRUD.STATUS.NORMAL
    //   callVmHook(crud, CRUD.HOOK.afterDeleteCancel)
    // },
    // /**
    //  * 取消新增/编辑
    //  */
    ...
    //     if (!callVmHook(crud, CRUD.HOOK.beforeAddCancel)) {
    //       return
    //     }
    //     crud.status.add = CRUD.STATUS.NORMAL
    //   }
    //   if (editStatus === CRUD.STATUS.PREPARED) {
    //     if (!callVmHook(crud, CRUD.HOOK.beforeEditCancel)) {
    //       return
    //     }
    ...
    //   crud.resetForm()
    //   if (addStatus === CRUD.STATUS.PREPARED) {
    //     callVmHook(crud, CRUD.HOOK.afterAddCancel)
    //   }
    //   if (editStatus === CRUD.STATUS.PREPARED) {
    ...
    //     callVmHook(crud, CRUD.HOOK.afterDelete)
    //     crud.refresh()
    //   }).catch(() => {
    //     if (delAll) {
    //       crud.delAllLoading = false
 ...
 * @param {string} tag
 */
function lookupCrud(vm, tag?: string) {
...
function presenter(crud) {
  ...
  // 被挂载前调用
  // onBeforeMount(()=>{
  //   crudState.$crud = crudState.$crud || {}
  //   let cruds = crudState.$options.cruds instanceof Function ? this.$options.cruds() : crud
